...
import { getExpenses, createExpense, updateExpense, getPayments, createPayment, getCustomers, getInvoices } from '@/lib/db'
import { idr } from '@/lib/format'
import type { Expense, Payment, Customer, Invoice } from '@/types'
import {
  DollarSign, Plus, Check, X, Loader2, TrendingUp, TrendingDown,
  CreditCard, Receipt, Filter, ArrowUpRight, ArrowDownRight
} from 'lucide-react'
...
export default function FinancePage() {
  ...
  const [expenseForm, setExpenseForm] = useState<Partial<Expense>>({
    category: 'other', expense_date: new Date().toISOString().split('T')[0], status: 'pending', amount: 0, currency: 'IDR'
  })
  const [paymentForm, setPaymentForm] = useState<Partial<Payment>>({
    method: 'bank_transfer', payment_date: new Date().toISOString().split('T')[0], amount: 0, currency: 'IDR'
  })
  ...
  const handleSaveExpense = async () => {
    ...
    try {
      ...
      setExpenseForm({ category: 'other', expense_date: new Date().toISOString().split('T')[0], status: 'pending', amount: 0, currency: 'IDR' })
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleSavePayment = async () => {
    ...
    setSaving(true)
    try {
      const created = await createPayment(paymentForm)
      setPayments([created as any, ...payments])
      // refresh invoices
      const inv = await getInvoices()
      setInvoices(inv)
      setShowPaymentForm(false)
      setPaymentForm({ method: 'bank_transfer', payment_date: new Date().toISOString().split('T')[0], amount: 0, currency: 'IDR' })
    } finally {
      setSaving(false)
    }
  }
  ...
  const byCategory = EXPENSE_CATEGORIES.reduce((acc, cat) => {
    acc[cat] = monthExpenses.filter(e => e.category === cat).reduce((s, e) => s + Number(e.amount), 0)
    return acc
    ...
    <>
      ...
      <div className="p-6 max-w-5xl space-y-6">
        ...
        <div className="flex items-center gap-3">
          <Label className="text-slate-500 text-sm">Viewing:</Label>
        ...
        <div className="flex gap-1 border-b border-slate-200">
          {[
            ...
            { id: 'payments' as Tab, label: `Payments Received (${monthPayments.length})` },
          ].map(({ id, label }) => (
        ...
        {tab === 'overview' && (
          <div className="grid grid-cols-2 gap-6">
            <Card>
              <CardHeader><CardTitle className="text-sm flex items-center gap-2"><TrendingDown className="w-4 h-4" />Expense Breakdown</CardTitle></CardHeader>
              <CardContent>
                {totalExpenses === 0 ? (
                    ...
                    {EXPENSE_CATEGORIES.filter(c => byCategory[c] > 0).sort((a, b) => byCategory[b] - byCategory[a]).map(cat => (
                      <div key={cat}>
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className="font-medium text-slate-800">{idr(byCategory[cat])}</span>
                        </div>
                        <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                          <div className="h-full bg-cyan-500 rounded-full" style={{ width: `${(byCategory[cat] / totalExpenses) * 100}%` }} />
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <Card>
              <CardHeader><CardTitle className="text-sm flex items-center gap-2"><CreditCard className="w-4 h-4" />Outstanding Invoices</CardTitle></CardHeader>
              <CardContent>
                {unpaidInvoices.length === 0 ? (
                  <p className="text-sm text-emerald-600 text-center py-4">✓ All invoices paid</p>
                ) : (
                  <div className="space-y-2">
                      ...
                      return (
                        <div key={inv.id} className="flex items-center gap-3 text-sm">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-slate-700 truncate">{customer?.name}</p>
                            <p className="text-xs text-slate-400">{inv.invoice_number} · Due {new Date(inv.due_date).toLocaleDateString()}</p>
                          </div>
                    ...
                    {unpaidInvoices.length > 8 && (
                      <p className="text-xs text-slate-400 text-center pt-1">+{unpaidInvoices.length - 8} more invoices</p>
                    )}
                  </div>
                )}
              </CardContent>
        ...
        {tab === 'expenses' && (
          <div className="space-y-4">
            <div className="flex justify-end">
              <Button onClick={() => setShowExpenseForm(true)}><Plus className="w-4 h-4 mr-1.5" /> Log Expense</Button>
            </div>

            {showExpenseForm && (
              <Card>
                <CardHeader><CardTitle className="text-sm">Log New Expense</CardTitle></CardHeader>
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label>Category *</Label>
                      <select className="w-full border rounded-md px-3 py-2 text-sm" value={expenseForm.category} onChange={e => setExpenseForm({ ...expenseForm, category: e.target.value as any })}>
            ...
            {monthExpenses.length === 0 ? (
              <div className="text-center py-12 text-slate-400 text-sm"><Receipt className="w-8 h-8 mx-auto mb-2 text-slate-200" />No expenses this month</div>
            ) : (
              <div className="space-y-2">
                {monthExpenses.map(e => (
                  <Card key={e.id}>
                ...
                <div className="flex justify-between pt-2 px-2 text-sm font-bold text-slate-700 border-t border-slate-200">
                  <span>Total</span>
                  <span className="text-red-600">{idr(totalExpenses)}</span>
                </div>
              </div>
            )}
          </div>
        ...
        {tab === 'payments' && (
          <div className="space-y-4">
            <div className="flex justify-end">
              <Button onClick={() => setShowPaymentForm(true)}><Plus className="w-4 h-4 mr-1.5" /> Record Payment</Button>
            </div>

            {showPaymentForm && (
              <Card>
                <CardHeader><CardTitle className="text-sm">Record Payment Received</CardTitle></CardHeader>
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label>Customer *</Label>
                      <select className="w-full border rounded-md px-3 py-2 text-sm" value={paymentForm.customer_id ?? ''} onChange={e => setPaymentForm({ ...paymentForm, customer_id: e.target.value, invoice_id: undefined })}>
                    ...
                    <div>
                      ...
                      <select className="w-full border rounded-md px-3 py-2 text-sm" value={paymentForm.invoice_id ?? ''} onChange={e => setPaymentForm({ ...paymentForm, invoice_id: e.target.value || undefined })}>
                        <option value="">Unlinked payment</option>
                        {invoices.filter(i => i.customer_id === paymentForm.customer_id && ['draft', 'sent', 'overdue'].includes(i.status)).map(i => (
                          <option key={i.id} value={i.id}>{i.invoice_number} — {idr(Number(i.total))}</option>
                        ))}
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label>Payment Method *</Label>
                      <select className="w-full border rounded-md px-3 py-2 text-sm" value={paymentForm.method} onChange={e => setPaymentForm({ ...paymentForm, method: e.target.value as any })}>
                        {PAYMENT_METHODS.map(m => <option key={m} value={m}>{m.replace('_', ' ')}</option>)}
                      </select>
                    </div>
                    <div><Label>Reference No.</Label><Input value={paymentForm.reference ?? ''} onChange={e => setPaymentForm({ ...paymentForm, reference: e.target.value })} placeholder="Bank ref, cheque no..." /></div>
                  </div>
                  <div><Label>Notes</Label><Input value={paymentForm.notes ?? ''} onChange={e => setPaymentForm({ ...paymentForm, notes: e.target.value })} /></div>
                  <div className="flex gap-2">
                    <Button className="bg-emerald-600 hover:bg-emerald-700 flex-1" onClick={handleSavePayment} disabled={saving}>
                      {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4 mr-1" />Record Payment</>}
                    </Button>
            ...
            {monthPayments.length === 0 ? (
              <div className="text-center py-12 text-slate-400 text-sm"><DollarSign className="w-8 h-8 mx-auto mb-2 text-slate-200" />No payments recorded this month</div>
            ) : (
              <div className="space-y-2">
                {monthPayments.map(p => {
                    ...
                    <Card key={p.id}>
                      <CardContent className="pt-3 pb-3">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <span className="font-medium text-slate-700">{customer?.name ?? 'Unknown'}</span>
                              <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{p.method.replace('_', ' ')}</span>
                              {(p as any).invoice && <span className="text-xs text-slate-400">{(p as any).invoice.invoice_number}</span>}
                            </div>
                          ...
                          <p className="font-bold text-emerald-600">{idr(Number(p.amount))}</p>
                        </div>
                      </CardContent>
                    </Card>
                  )
                ...
                <div className="flex justify-between pt-2 px-2 text-sm font-bold text-slate-700 border-t border-slate-200">
                  ...
                  <span className="text-emerald-600">{idr(totalIncoming)}</span>
                </div>
              </div>
            )}
          </div>
