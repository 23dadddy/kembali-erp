...
export default function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!invoice || !customer) return <><Topbar title="Invoice" /><div className="p-6 text-slate-400">Invoice not found</div></>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {new Date(invoice.due_date) < new Date() && invoice.status !== 'paid' && (
              <span className="text-xs text-red-500 flex items-center gap-1"><AlertCircle className="w-3 h-3" />Overdue</span>
            )}
          </div>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <table className="w-full mb-6">
              <thead>
                <tr className="bg-cyan-500 text-white">
                  <th className="text-left text-xs font-semibold p-3 rounded-tl-lg">Description</th>
                  <th className="text-center text-xs font-semibold p-3">Size</th>
                  <th className="text-center text-xs font-semibold p-3">Qty</th>
                  <th className="text-right text-xs font-semibold p-3">Unit Price</th>
                  <th className="text-right text-xs font-semibold p-3 rounded-tr-lg">Total</th>
                </tr>
              </thead>
              <tbody>
            ...
            <div className="mt-6 bg-emerald-50 border border-emerald-100 rounded-xl p-4">
              <p className="text-xs font-semibold text-emerald-700 mb-1">Payment Instructions</p>
              <p className="text-sm text-slate-600">Bank Transfer · BCA · Account: 123-456-7890 · PT Kembali Air Bali</p>
              <p className="text-xs text-slate-400 mt-1">Please include <strong>{invoice.invoice_number}</strong> as your payment reference.</p>
            </div>
          </CardContent>
        </Card>
      </div>
