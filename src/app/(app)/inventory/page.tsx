...
function StockTab() {
  ...
  return (
    <div className="p-6 space-y-6">
      <Card>
        <CardHeader><CardTitle className="text-sm font-semibold text-slate-500 uppercase tracking-wide">Bottle Flow</CardTitle></CardHeader>
        <CardContent>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card><CardContent className="pt-5"><div className="flex items-center justify-between"><div><p className="text-sm text-slate-500">Total 350ml</p><p className="text-3xl font-bold mt-1">{loading ? '—' : total350}</p></div><Package className="w-8 h-8 text-slate-200" /></div></CardContent></Card>
        <Card><CardContent className="pt-5"><div className="flex items-center justify-between"><div><p className="text-sm text-slate-500">Total 750ml</p><p className="text-3xl font-bold mt-1">{loading ? '—' : total750}</p></div><Package className="w-8 h-8 text-slate-200" /></div></CardContent></Card>
      </div>
      <div className="space-y-2">
        <div className="flex items-center justify-between"><h2 className="font-semibold text-slate-700">Inventory by Status</h2><p className="text-xs text-slate-400">Click a count to adjust</p></div>
          ...
          <div className="grid gap-3">
            {inventoryConfig.map(({ status, label, description, color, dot }) => (
              <div key={status} className={`flex items-center gap-4 p-4 rounded-xl border ${color}`}>
                ...
                <div className="flex-1"><p className="font-medium text-sm text-slate-800">{label}</p><p className="text-xs text-slate-500">{description}</p></div>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-sm"><DialogHeader><DialogTitle>Adjust Inventory</DialogTitle></DialogHeader>
          {editRow && <div className="space-y-4 py-2">
            <p className="text-sm text-slate-500">Setting <strong>{editRow.bottle_size}</strong> — <strong>{inventoryConfig.find(c => c.status === editRow.status)?.label}</strong></p>
            <div className="space-y-1"><Label>Quantity</Label><Input type="number" min="0" value={editQty} onChange={e => setEditQty(e.target.value)} /></div>
            <div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setEditOpen(false)}>Cancel</Button><Button className="bg-cyan-600 hover:bg-cyan-700" onClick={handleSave} disabled={saving}>{saving && <Loader2 className="w-4 h-4 animate-spin mr-2" />}Save</Button></div>
          </div>}
        </DialogContent>
      </Dialog>
    </div>
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm"><p className="text-xs text-slate-400 mb-1">350ml Filled</p><p className="text-2xl font-bold text-slate-800">{inv350filled.toLocaleString()}</p></div>
        <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm"><p className="text-xs text-slate-400 mb-1">750ml Filled</p><p className="text-2xl font-bold text-slate-800">{inv750filled.toLocaleString()}</p></div>
        <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm"><p className="text-xs text-slate-400 mb-1">Outstanding (all customers)</p><p className="text-2xl font-bold text-slate-800">{(totalOut350 + totalOut750).toLocaleString()}</p><p className="text-xs text-slate-400 mt-1">{totalOut350} × 350ml · {totalOut750} × 750ml</p></div>
        <div className={`border rounded-2xl p-4 shadow-sm ${chargeableCount > 0 ? 'bg-red-50 border-red-200' : 'bg-white border-slate-100'}`}><p className={`text-xs mb-1 ${chargeableCount > 0 ? 'text-red-400' : 'text-slate-400'}`}>Chargeable Lost</p><p className={`text-xl font-bold ${chargeableCount > 0 ? 'text-red-700' : 'text-slate-800'}`}>{fmt(totalChargeable)}</p><p className={`text-xs mt-1 ${chargeableCount > 0 ? 'text-red-400' : 'text-slate-400'}`}>{chargeableCount} customer{chargeableCount !== 1 ? 's' : ''} over threshold</p></div>
      </div>
      <div className="bg-cyan-50 border border-cyan-200 rounded-2xl p-4"><p className="text-sm font-semibold text-cyan-800 mb-1">Bottle Loss Policy</p><p className="text-xs text-cyan-700">Customers may keep up to <strong>8%</strong> of delivered bottles outstanding without charge. Bottles beyond this threshold are chargeable at replacement cost (350ml: Rp 6,000 · 750ml: Rp 10,000).</p></div>
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" /><input className="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 rounded-xl" placeholder="Search customers..." value={search} onChange={e => setSearch(e.target.value)} /></div>
        <div className="flex gap-1 bg-slate-100 rounded-xl p-1">{(['all', 'chargeable', 'outstanding'] as const).map(f => <button key={f} onClick={() => setFilterView(f)} className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors ${filterView === f ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{f === 'all' ? 'All' : f === 'chargeable' ? '⚠ Chargeable' : 'Outstanding'}</button>)}</div>
        <select className="border border-slate-200 rounded-xl px-3 py-2 text-sm bg-white" value={sortBy} onChange={e => setSortBy(e.target.value as any)}><option value="chargeable">Sort: Chargeable first</option><option value="outstanding">Sort: Most outstanding</option><option value="name">Sort: Name A–Z</option></select>
      </div>
      {loading ? <div className="flex justify-center py-10"><Loader2 className="w-6 h-6 animate-spin text-slate-300" /></div>
        : filtered.length === 0 ? <div className="text-center py-16 text-slate-400"><Package className="w-10 h-10 mx-auto mb-3 text-slate-200" /><p>{search || filterView !== 'all' ? 'No matching customers' : 'No bottle data yet'}</p></div>
        : (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-slate-100 bg-slate-50"><th className="text-left px-5 py-3 text-xs font-medium text-slate-400 uppercase">Customer</th><th className="text-right px-4 py-3 text-xs font-medium text-slate-400 uppercase">350ml Out</th><th className="text-right px-4 py-3 text-xs font-medium text-slate-400 uppercase">750ml Out</th><th className="text-right px-4 py-3 text-xs font-medium text-slate-400 uppercase">350ml Chargeable</th><th className="text-right px-4 py-3 text-xs font-medium text-slate-400 uppercase">750ml Chargeable</th><th className="text-right px-5 py-3 text-xs font-medium text-slate-400 uppercase">Charge Amount</th></tr></thead>
...
const TABS: { id: Tab; label: string }[] = [
  { id: 'stock', label: 'Bottle Stock' },
  { id: 'bottle-tracking', label: 'Customer Bottle Tracking' },
