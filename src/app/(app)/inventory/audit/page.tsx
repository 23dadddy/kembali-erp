...
export default function InventoryAuditPage() {
  ...
  const statusConfig: Record<string, { color: string; labelKey: 'audit_in_progress' | 'audit_completed' | 'audit_discrepancy_found' }> = {
    in_progress: { color: 'bg-amber-100 text-amber-700', labelKey: 'audit_in_progress' },
    completed: { color: 'bg-emerald-100 text-emerald-700', labelKey: 'audit_completed' },
    discrepancy_found: { color: 'bg-red-100 text-red-600', labelKey: 'audit_discrepancy_found' },
  }

  return (
    <>
      <Topbar title={t('audit_title')} />
        ...
        <div className="w-72 border-r border-slate-200 bg-white flex flex-col flex-shrink-0">
          <div className="p-4 border-b border-slate-100">
            <button onClick={() => setShowNewAudit(true)}
              ...
              <Plus className="w-4 h-4" /> {t('audit_start')}
            </button>
          </div>
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="text-center py-12 text-slate-400 px-4">
                ...
                <p className="text-sm">{t('audit_no_audits')}</p>
              </div>
            ) : audits.map(audit => {
          ...
          {!selectedAudit ? (
            <div className="flex items-center justify-center h-full text-slate-400">
              <div className="text-center">
                ...
                <p className="font-medium">{t('audit_select_or_start')}</p>
              </div>
            </div>
          ) : (
            <div className="p-6 space-y-5">
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-lg font-bold text-slate-800">{t('audit_stock_count')}</h2>
                    ...
                    <span className={`inline-block mt-2 text-xs px-2.5 py-1 rounded-full font-medium ${statusConfig[selectedAudit.status]?.color ?? ''}`}>
                      {statusConfig[selectedAudit.status] ? t(statusConfig[selectedAudit.status].labelKey) : ''}
                    </span>
                  </div>
                  {selectedAudit.status === 'in_progress' && (
                    <button onClick={saveAuditItems} disabled={saving}
                      ...
                      {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><CheckCircle2 className="w-4 h-4" />{t('audit_complete')}</>}
                    </button>
                  )}
                </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-slate-100">
                  <h3 className="font-semibold text-slate-800">{t('audit_count_sheet')}</h3>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50">
                        <th className="text-left px-5 py-3 text-xs font-medium text-slate-400 uppercase">{t('audit_size')}</th>
                        <th className="text-left px-4 py-3 text-xs font-medium text-slate-400 uppercase">{t('status')}</th>
                        <th className="text-right px-4 py-3 text-xs font-medium text-slate-400 uppercase">{t('audit_system_qty')}</th>
                        <th className="text-right px-4 py-3 text-xs font-medium text-slate-400 uppercase">{t('audit_actual_count')}</th>
                        <th className="text-right px-5 py-3 text-xs font-medium text-slate-400 uppercase">{t('audit_difference')}</th>
                      </tr>
                    </thead>
      ...
      {showNewAudit && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
            <h3 className="font-bold text-slate-800 text-lg">{t('audit_start_modal_title')}</h3>
            <div>
              <label className="text-xs font-medium text-slate-600 block mb-1">{t('audit_warehouse')} *</label>
            ...
            <div>
              <label className="text-xs font-medium text-slate-600 block mb-1">{t('audit_performed_by')}</label>
              <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm"
            ...
            <div className="flex gap-2">
              <button onClick={createAudit} disabled={saving || !newAuditForm.warehouse_id}
                ...
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4" />{t('audit_start')}</>}
              </button>
              <button onClick={() => setShowNewAudit(false)} className="border border-slate-200 px-4 py-2.5 rounded-xl text-sm hover:bg-slate-50">{t('cancel')}</button>
            </div>
          </div>
        </div>
      )}
