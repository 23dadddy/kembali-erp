    ...
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {counts.pending > 0 && (
            <button onClick={dispatchAll} className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
              <Navigation className="w-4 h-4" /> {t('dispatch_dispatch_all')} ({counts.pending})
            </button>
          )}
          <button onClick={handleGenerateDeliveries} disabled={generating} className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 disabled:opacity-60 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
            {generating ? <Loader2 className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4" />} {t('dispatch_generate_today')}
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {selectedDate !== new Date().toISOString().split('T')[0] && (
          <button onClick={() => { const td = new Date().toISOString().split('T')[0]; setSelectedDate(td); setSelectedDay(today) }} className="text-xs text-cyan-600 hover:text-cyan-700 font-medium">{t('back_to_today')}</button>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-slate-700">{t('dispatch_routes_on')} {selectedDay.slice(0, 3)}</h2>
            <Dialog open={routeOpen} onOpenChange={setRouteOpen}>
              <DialogTrigger className="inline-flex items-center gap-1.5 rounded-lg border text-sm px-3 py-1.5 hover:bg-slate-50">
                <Plus className="w-3.5 h-3.5" /> {t('dispatch_new_route')}
              </DialogTrigger>
              <DialogContent className="max-w-md">
                <DialogHeader><DialogTitle>{t('dispatch_new_route')}</DialogTitle></DialogHeader>
                <div className="space-y-4 py-2">
                  <div className="space-y-1">
                    <Label>{t('dispatch_route_name')} *</Label>
                    <Input placeholder="e.g. Seminyak North" value={routeForm.name} onChange={(e) => setRouteForm({ ...routeForm, name: e.target.value })} />
                  </div>
                  <div className="space-y-1">
                    <Label>{t('dispatch_driver')}</Label>
                    <Select value={routeForm.driver_id} onValueChange={(v) => setRouteForm({ ...routeForm, driver_id: v ?? '' })}>
                      <SelectTrigger><SelectValue placeholder={t('dispatch_select_driver')} /></SelectTrigger>
                      <SelectContent>{drivers.map((d) => <SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-2">
                    <Label>{t('dispatch_active_days')}</Label>
                  ...
                  <div className="flex justify-end gap-2">
                    <Button variant="outline" onClick={() => setRouteOpen(false)}>{t('cancel')}</Button>
                    <Button className="bg-cyan-600 hover:bg-cyan-700" onClick={handleCreateRoute} disabled={savingRoute || !routeForm.name}>
                      {savingRoute && <Loader2 className="w-4 h-4 animate-spin mr-2" />} {t('dispatch_new_route')}
                    </Button>
                  </div>
                </div>
              </DialogContent>
          ...
          {loading ? <div className="text-center py-8"><Loader2 className="w-5 h-5 animate-spin text-slate-300 mx-auto" /></div>
            : todayRoutes.length === 0 ? (
              <div className="bg-white rounded-xl border p-6 text-center">
                ...
                <p className="text-sm text-slate-400 font-medium">{t('dispatch_no_routes')} {selectedDay}</p>
              </div>
              ...
              <div key={route.id} className="bg-white rounded-xl border p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 flex items-center gap-1 mt-0.5"><Users className="w-3 h-3" />{(route.driver as any)?.name ?? t('dispatch_unassigned')}</p>
                  </div>
                  <Badge className="bg-blue-100 text-blue-700 text-xs">{route.stops?.length ?? 0} {t('dispatch_stops')}</Badge>
                </div>
        ...
        <div className="lg:col-span-2 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-slate-700">{t('dispatch_deliveries_on')} {selectedDate}</h2>
            <Dialog open={deliveryOpen} onOpenChange={setDeliveryOpen}>
              <DialogTrigger className="inline-flex items-center gap-2 rounded-md bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-medium px-4 py-2 transition-colors">
                <Plus className="w-4 h-4" /> {t('dispatch_add_delivery')}
              </DialogTrigger>
              <DialogContent className="max-w-md">
                <DialogHeader><DialogTitle>{t('dispatch_new_delivery')}</DialogTitle></DialogHeader>
                <div className="space-y-4 py-2">
                  <div className="space-y-1">
                    <Label>{t('orders_customer')} *</Label>
                    <Select value={deliveryForm.customer_id} onValueChange={(v) => setDeliveryForm({ ...deliveryForm, customer_id: v ?? '' })}>
                      <SelectTrigger><SelectValue placeholder={t('dispatch_select_customer')} /></SelectTrigger>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1"><Label>{t('dispatch_qty_350')}</Label><Input type="number" min="0" value={deliveryForm.delivered_350ml} onChange={(e) => setDeliveryForm({ ...deliveryForm, delivered_350ml: parseInt(e.target.value) || 0 })} /></div>
                    <div className="space-y-1"><Label>{t('dispatch_qty_750')}</Label><Input type="number" min="0" value={deliveryForm.delivered_750ml} onChange={(e) => setDeliveryForm({ ...deliveryForm, delivered_750ml: parseInt(e.target.value) || 0 })} /></div>
                  </div>
                  <div className="flex justify-end gap-2">
                    <Button variant="outline" onClick={() => setDeliveryOpen(false)}>{t('cancel')}</Button>
                    <Button className="bg-cyan-600 hover:bg-cyan-700" onClick={handleCreateDelivery} disabled={savingDelivery || !deliveryForm.customer_id}>
                      {savingDelivery && <Loader2 className="w-4 h-4 animate-spin mr-2" />} {t('create')}
                    </Button>
                  </div>
                </div>
              </DialogContent>
          ...
          <div className="grid grid-cols-4 gap-3">
            {(Object.entries(statusConfig) as [string, typeof statusConfig[keyof typeof statusConfig]][]).map(([key, { icon: Icon }]) => (
              <div key={key} className="bg-white rounded-lg border p-3 text-center">
                ...
                <div className="text-xs text-slate-500">{t(key as 'pending' | 'in_transit' | 'completed' | 'failed')}</div>
              </div>
            ))}
          </div>
            ...
            {loading ? <div className="bg-white rounded-xl border p-8 text-center"><Loader2 className="w-6 h-6 animate-spin text-slate-300 mx-auto" /></div>
              : deliveries.length === 0 ? (
                <div className="bg-white rounded-xl border p-8 text-center">
                  ...
                  <p className="text-sm font-medium text-slate-400">{t('dispatch_no_deliveries')}</p>
                </div>
              ) : deliveries.map((d) => {
                ...
                return (
                  <div key={d.id} className="bg-white rounded-xl border p-4 flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      {d.status === 'pending' && <button onClick={() => markInTransit(d.id)} className="text-xs text-blue-600 hover:text-blue-800 font-medium px-2 py-1 rounded-lg hover:bg-blue-50">{t('start')}</button>}
                      {d.status === 'in_transit' && (
                        <div className="flex gap-1">
                          <button onClick={() => markCompleted(d.id)} className="text-xs text-emerald-600 hover:text-emerald-800 font-medium px-2 py-1 rounded-lg hover:bg-emerald-50">✓ {t('done')}</button>
                          <button onClick={() => markFailed(d.id)} className="text-xs text-red-500 hover:text-red-700 font-medium px-2 py-1 rounded-lg hover:bg-red-50">✕ {t('dispatch_fail')}</button>
                        </div>
                      )}
                      {(d.status === 'pending' || d.status === 'in_transit') && (
                        <Link href={`/deliver/${d.id}`} className="flex items-center gap-1 bg-cyan-600 text-white text-xs px-3 py-1.5 rounded-lg hover:bg-cyan-700">
                          {t('orders_log')} <ExternalLink className="w-3 h-3" />
                        </Link>
                      )}
                      {d.status === 'completed' && <span className="text-xs text-emerald-600 flex items-center gap-1"><CheckCircle2 className="w-3 h-3" />{d.signature_confirmed_by ?? t('done')}</span>}
                      {d.status === 'failed' && <span className="text-xs text-red-500 font-medium">✕ {t('failed')}</span>}
                    </div>
                  </div>
                )
              })}
      ...
      <Card>
        <CardHeader><CardTitle className="text-base flex items-center gap-2"><RotateCcw className="w-4 h-4 text-cyan-600" />{t('dispatch_bottle_totals')} — {selectedDate}</CardTitle></CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { labelKey: 'dispatch_delivered_350' as const, value: deliveries.reduce((s, d) => s + d.delivered_350ml, 0), color: 'text-emerald-600' },
              { labelKey: 'dispatch_delivered_750' as const, value: deliveries.reduce((s, d) => s + d.delivered_750ml, 0), color: 'text-emerald-600' },
              { labelKey: 'dispatch_collected_350' as const, value: deliveries.reduce((s, d) => s + d.collected_350ml, 0), color: 'text-amber-600' },
              { labelKey: 'dispatch_collected_750' as const, value: deliveries.reduce((s, d) => s + d.collected_750ml, 0), color: 'text-amber-600' },
            ].map(({ labelKey, value, color }) => (
              <div key={labelKey} className="bg-slate-50 rounded-lg p-4 text-center">
      ...
      <div className="w-72 border-r border-slate-200 bg-white flex flex-col flex-shrink-0">
        <div className="p-4 border-b border-slate-100 space-y-2">
          <button onClick={() => setShowRouteForm(true)} className="w-full flex items-center justify-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white py-2 rounded-xl text-sm font-medium transition-colors">
            <Plus className="w-4 h-4" /> {t('dispatch_new_route')}
          </button>
        </div>
        {loading ? <div className="flex justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-slate-300" /></div> : (
          <div className="flex-1 overflow-y-auto">
            {routes.length === 0 ? (
              <div className="text-center py-12 text-slate-400 px-4"><Route className="w-8 h-8 mx-auto mb-2 text-slate-200" /><p className="text-sm">{t('dispatch_no_routes_yet')}</p></div>
            ) : routes.map(route => (
      ...
      <div className="flex-1 overflow-hidden bg-slate-50 flex flex-col">
        {!selectedRoute ? (
          <div className="flex items-center justify-center h-full text-slate-400">
            <div className="text-center"><MapPin className="w-12 h-12 mx-auto mb-3 text-slate-200" /><p className="font-medium">{t('dispatch_select_route')}</p></div>
          </div>
        ) : (
          ...
          <div className="flex-1 overflow-y-auto p-6 pt-4 space-y-6">
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex flex-col gap-2 items-end">
                  <button onClick={generateDeliveries} disabled={generating || stops.length === 0} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors">
                    {generating ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />} {t('dispatch_generate_deliveries')}
                  </button>
                  <button onClick={optimizeRoute} disabled={optimizing || stops.length < 2} className="flex items-center gap-2 bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors">
                    {optimizing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} {t('dispatch_optimize')}
                  </button>
                </div>
              </div>
              <div className="mt-3 flex items-center gap-2">
                <label className="text-xs text-slate-500 whitespace-nowrap font-medium">{t('dispatch_start_from')}</label>
            ...
            {optimizeResult && (
              <div className="bg-violet-50 border border-violet-200 rounded-2xl p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-violet-800 text-sm">{t('dispatch_route_optimized')}</p>
                    <p className="text-xs text-violet-600 mt-0.5">{t('dispatch_total')} <strong>{optimizeResult.totalDistance}</strong> · <strong>{optimizeResult.totalDuration}</strong></p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <a href={`https://www.google.com/maps/dir/${encodeURIComponent(startAddress)}/${optimizeResult.optimizedStops?.map((s: any) => encodeURIComponent(`${s.customer?.address}, ${s.customer?.city}, Bali`)).join('/')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-violet-600 hover:bg-violet-700 text-white px-3 py-1.5 rounded-lg text-xs font-medium">
                    <ExternalLink className="w-3.5 h-3.5" /> {t('dispatch_open_maps')}
                  </a>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm">
              <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                <div><h3 className="font-semibold text-slate-800">{t('dispatch_route_stops')}</h3><p className="text-xs text-slate-400 mt-0.5">{stops.length} {t('dispatch_stops')} · {t('dispatch_in_delivery_order')}</p></div>
                <button onClick={() => setShowStopForm(!showStopForm)} className="flex items-center gap-1.5 text-sm text-cyan-600 hover:text-cyan-700 font-medium"><Plus className="w-4 h-4" /> {t('dispatch_add_stop')}</button>
              </div>
              {showStopForm && (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      ...
                      <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white" value={stopForm.customer_id} onChange={e => setStopForm({ ...stopForm, customer_id: e.target.value })}>
                        <option value="">{t('dispatch_select_customer')}</option>
                        {customers.map(c => <option key={c.id} value={c.id}>{c.name} — {c.city}</option>)}
                      </select>
                    </div>
                    <div><label className="text-xs font-medium text-slate-600 block mb-1">{t('dispatch_stop_order')}</label><input type="number" min="1" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white" value={stopForm.stop_order} onChange={e => setStopForm({ ...stopForm, stop_order: Number(e.target.value) })} /></div>
                    <div><label className="text-xs font-medium text-slate-600 block mb-1">{t('dispatch_est_arrival')}</label><input type="time" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white" value={stopForm.estimated_arrival} onChange={e => setStopForm({ ...stopForm, estimated_arrival: e.target.value })} /></div>
                  </div>
                  <div><label className="text-xs font-medium text-slate-600 block mb-1">{t('notes')}</label><input className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white" placeholder="Parking, access, special instructions..." value={stopForm.notes} onChange={e => setStopForm({ ...stopForm, notes: e.target.value })} /></div>
                  <div className="flex gap-2">
                    <button onClick={addStop} disabled={saving || !stopForm.customer_id} className="flex-1 bg-cyan-600 hover:bg-cyan-700 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4" />{t('dispatch_add_stop')}</>}</button>
              ...
              <div className="divide-y divide-slate-50">
                {stops.length === 0 ? <div className="px-5 py-10 text-center text-slate-400"><MapPin className="w-6 h-6 mx-auto mb-2 text-slate-200" /><p className="text-sm">{t('dispatch_no_stops')}</p></div>
                  : stops.map((stop, idx) => (
      ...
      {showRouteForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-4">
            <h3 className="font-bold text-slate-800 text-lg">{t('dispatch_new_route')}</h3>
            <div><label className="text-xs font-medium text-slate-600 block mb-1">{t('dispatch_route_name')} *</label><input className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm" placeholder="e.g. Seminyak Morning Run" value={routeForm.name} onChange={e => setRouteForm({ ...routeForm, name: e.target.value })} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs font-medium text-slate-600 block mb-1">{t('dispatch_driver')}</label><select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm" value={routeForm.driver_id} onChange={e => setRouteForm({ ...routeForm, driver_id: e.target.value })}><option value="">{t('dispatch_unassigned')}</option>{staff.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
              <div><label className="text-xs font-medium text-slate-600 block mb-1">{t('dispatch_vehicle')}</label><select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm" value={routeForm.vehicle_id} onChange={e => setRouteForm({ ...routeForm, vehicle_id: e.target.value })}><option value="">{t('dispatch_unassigned')}</option>{vehicles.map(v => <option key={v.id} value={v.id}>{v.name} ({v.plate_number})</option>)}</select></div>
              <div><label className="text-xs font-medium text-slate-600 block mb-1">{t('dispatch_est_duration')}</label><input type="number" min="0" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm" value={routeForm.estimated_duration_mins} onChange={e => setRouteForm({ ...routeForm, estimated_duration_mins: e.target.value })} /></div>
              <div><label className="text-xs font-medium text-slate-600 block mb-1">{t('dispatch_est_distance')}</label><input type="number" min="0" step="0.1" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm" value={routeForm.estimated_km} onChange={e => setRouteForm({ ...routeForm, estimated_km: e.target.value })} /></div>
            </div>
            <div>
              <label className="text-xs font-medium text-slate-600 block mb-1.5">{t('dispatch_active_days')}</label>
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={saveRoute} disabled={saving || !routeForm.name} className="flex-1 bg-cyan-600 hover:bg-cyan-700 disabled:opacity-50 text-white px-4 py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-2">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4" />{t('dispatch_new_route')}</>}</button>
              <button onClick={() => setShowRouteForm(false)} className="border border-slate-200 px-4 py-2.5 rounded-xl text-sm hover:bg-slate-50">{t('cancel')}</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div className="flex h-[calc(100vh-113px)]">
      <div className="flex-1 flex flex-col p-6">
        ...
        <div className="flex gap-3 mb-4">
          {[
            { labelKey: 'dispatch_calendar_total' as const, value: deliveries.length, color: 'text-slate-800' },
            { labelKey: 'pending' as const, value: deliveries.filter(d => d.status === 'pending').length, color: 'text-amber-600' },
            { labelKey: 'completed' as const, value: deliveries.filter(d => d.status === 'completed').length, color: 'text-emerald-600' },
            { labelKey: 'failed' as const, value: deliveries.filter(d => d.status === 'failed').length, color: 'text-red-600' },
          ].map(s => (
            <div key={s.labelKey} className="bg-white border border-slate-100 rounded-xl px-4 py-2 shadow-sm">
      ...
      <div className="w-72 border-l border-slate-200 bg-white flex flex-col flex-shrink-0">
        <div className="p-4 border-b border-slate-100">
          <p className="font-semibold text-slate-800">{selected ? new Date(selected + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }) : t('dispatch_select_day')}</p>
          {selected && <p className="text-xs text-slate-400 mt-0.5">{selectedDeliveries.length} {selectedDeliveries.length !== 1 ? t('dispatch_deliveries_count') : t('dispatch_delivery_count')}</p>}
        </div>
        <div className="flex-1 overflow-y-auto">
          {selectedDeliveries.length === 0 ? (
            <div className="text-center py-12 text-slate-400"><Truck className="w-8 h-8 mx-auto mb-2 text-slate-200" /><p className="text-sm">{t('dispatch_no_deliveries_day')}</p></div>
            ...
            <div key={d.id} className="w-full text-left px-4 py-3 border-b border-slate-50">
              ...
              <div className="flex items-center gap-3 mt-0.5 text-xs text-slate-400">
                <span className="flex items-center gap-0.5"><Package className="w-3 h-3" />{(d.delivered_350ml ?? 0) + (d.delivered_750ml ?? 0)} {t('dispatch_bottles')}</span>
...
// ─── MAIN PAGE ─────────────────────────────────────────────────────────────────
const DISPATCH_TAB_KEYS = [
  { id: 'live' as Tab, labelKey: 'dispatch_live' as const, icon: Truck },
  { id: 'routes' as Tab, labelKey: 'dispatch_routes' as const, icon: Route },
  { id: 'calendar' as Tab, labelKey: 'dispatch_calendar' as const, icon: Calendar },
]
...
export default function DispatchPage() {
  ...
  return (
    <>
      <Topbar title="dispatch_title" titleIsKey />
      {/* Tab bar */}
      <div className="bg-white border-b border-slate-200 px-6">
        <div className="flex gap-1">
          {DISPATCH_TAB_KEYS.map(({ id, labelKey, icon: Icon }) => (
